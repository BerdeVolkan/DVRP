...
3. `DVRP_algo.py` — OR-Tools solver pipeline:
   ...
   - `convert_ortools_solution_to_dvrp()` — translates OR-Tools index routes back to dvrpsim's `{vehicles: {next_visits: [...]}, orders: {...}}` format. Note: a vehicle whose OR-Tools route starts back at `DEPOT` while it already has `loaded_orders` (`skip_outer` branch) is dropped from the result entirely — a known latent edge case that can make that vehicle's orders temporarily disappear from the result (see Reassigning Rate caveat below).
